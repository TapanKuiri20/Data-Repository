    ...
    try {
        ...
        if (user) {
            req.session.destroy((err) => {
                ...
                res.render('home'); // Redirect to home or another appropriate page
            });
        } else {
...
app.post('/deleteAccount', async (req, res) => {
    try {
        ...
        req.session.destroy((err) => {
            ...
            res.render('home'); // Redirect to signup or another appropriate page
        });
    } catch (error) {
...
app.get("/upload", async (req, res) => {
    if (req.session.user) {
        try {
            ...
            res.render("upload", { userFiles: files });
        } catch (err) {
            console.error(err);
            res.status(500).send("Internal Server Error");
        }
